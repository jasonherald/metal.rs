...
impl MetalLayer {
    ...
    pub fn layer() -> Option<Self> {
        unsafe {
            let class = metal_sys::Class::get("CAMetalLayer")?;
    ...
    pub fn set_device(&self, device: &metal::Device) {
        unsafe {
    ...
    pub fn pixel_format(&self) -> metal::PixelFormat {
    ...
    pub fn set_pixel_format(&self, pixel_format: metal::PixelFormat) {
        unsafe {
    ...
    pub fn residency_set(&self) -> Option<metal::ResidencySet> {
        unsafe {
            let ptr: *mut c_void = msg_send_0(self.as_ptr(), sel!(residencySet));
            metal::ResidencySet::from_raw(ptr)
        }
    }
}
