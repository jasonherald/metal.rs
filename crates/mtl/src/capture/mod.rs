...
use mtl_foundation::{Referencing, Url};
...
impl CaptureDescriptor {
    ...
    pub fn alloc() -> Option<Self> {
        unsafe {
            let cls = mtl_sys::Class::get("MTLCaptureDescriptor")?;
...
impl CaptureManager {
    ...
    pub fn shared() -> Option<Self> {
        unsafe {
            let cls = mtl_sys::Class::get("MTLCaptureManager")?;
    ...
    pub fn start_capture(&self, descriptor: &CaptureDescriptor) -> Result<(), CaptureError> {
        unsafe {
            ...
            if success {
                ...
                if !error.is_null() {
                    let code: mtl_foundation::Integer =
                        mtl_sys::msg_send_0(error as *const c_void, sel!(code));
