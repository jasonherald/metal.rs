...
use mtl_sys::{msg_send_0, msg_send_2, msg_send_3, msg_send_4, sel};
...
fn generic_error() -> mtl_foundation::Error {
...
impl Compiler {
    ...
    ) -> Result<BinaryFunction, mtl_foundation::Error> {
        unsafe {
            let mut error: *mut c_void = std::ptr::null_mut();
    ...
    pub fn new_dynamic_library_from_library(
        &self,
        library: &Library,
    ) -> Result<*mut c_void, mtl_foundation::Error> {
        unsafe {
            let mut error: *mut c_void = std::ptr::null_mut();
            let ptr: *mut c_void = msg_send_2(
                self.as_ptr(),
    ...
    where
        F: Fn(Option<BinaryFunction>, Option<mtl_foundation::Error>) + Send + 'static,
    {
        let block =
    ...
    where
        F: Fn(Option<DynamicLibrary>, Option<mtl_foundation::Error>) + Send + 'static,
    {
        let block =
            mtl_sys::TwoArgBlock::from_fn(move |lib_ptr: *mut c_void, err_ptr: *mut c_void| {
    ...
    pub fn new_machine_learning_pipeline_state(
        ...
        options: Option<&CompilerTaskOptions>,
    ) -> Result<MachineLearningPipelineState, mtl_foundation::Error> {
        unsafe {
            let mut error: *mut c_void = std::ptr::null_mut();
            let options_ptr = options.map_or(std::ptr::null(), |o| o.as_ptr());
            let ptr: *mut c_void = msg_send_3(
    ...
    where
        F: Fn(Option<MachineLearningPipelineState>, Option<mtl_foundation::Error>)
            + Send
            + 'static,
    {
        let block =
