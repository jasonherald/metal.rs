...
impl PipelineDataSetSerializerDescriptor {
    ...
    pub fn new() -> Option<Self> {
        unsafe {
            let class = metal_sys::Class::get("MTL4PipelineDataSetSerializerDescriptor")?;
...
impl PipelineDataSetSerializer {
    ...
    pub fn serialize_as_archive_and_flush_to_url(
        &self,
        url: *const c_void,
    ) -> Result<bool, metal_foundation::Error> {
        unsafe {
    ...
    pub fn serialize_as_pipelines_script(&self) -> Result<*mut c_void, metal_foundation::Error> {
        unsafe {
            let mut error: *mut c_void = std::ptr::null_mut();
