...
use metal_foundation::{Referencing, TimeInterval, UInteger};
...
impl CommandBufferDescriptor {
    ...
    pub fn new() -> Option<Self> {
        unsafe {
            let class = metal_sys::class!(MTLCommandBufferDescriptor);
...
impl CommandBuffer {
    ...
    pub unsafe fn present_drawable_at_time(&self, drawable: *const c_void, time: TimeInterval) {
        unsafe {
            metal_sys::msg_send_2::<(), *const c_void, TimeInterval>(
                self.as_ptr(),
