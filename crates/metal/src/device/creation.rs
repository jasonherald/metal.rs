...
pub fn system_default() -> Option<Device> {
    unsafe {
        ...
        let _: *mut c_void = metal_sys::msg_send_0(ptr, metal_sys::sel!(retain));
        Device::from_raw(ptr)
    }
}
...
pub fn copy_all_devices() -> Vec<Device> {
    unsafe {
        ...
        // Get the count
        let count: usize = metal_sys::msg_send_0(array_ptr, metal_sys::sel!(count));

        let mut devices = Vec::with_capacity(count);
        for i in 0..count {
            let device_ptr: *mut c_void =
                metal_sys::msg_send_1(array_ptr, metal_sys::sel!(objectAtIndex:), i);
        ...
        metal_sys::msg_send_0::<()>(array_ptr, metal_sys::sel!(release));
...
{
    use metal_sys::MTLCopyAllDevicesWithObserver;
...
fn parse_notification_name(ns_string: *mut c_void) -> DeviceNotificationName {
    ...
    unsafe {
        let c_str: *const i8 = metal_sys::msg_send_0(ns_string, metal_sys::sel!(UTF8String));
        if c_str.is_null() {
...
pub fn remove_device_observer(observer: DeviceObserver) {
    use metal_sys::MTLRemoveDeviceObserver;
    unsafe {
