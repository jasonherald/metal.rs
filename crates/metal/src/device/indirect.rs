...
use metal_sys::{msg_send_1, msg_send_2, msg_send_3, sel};
...
impl Device {
    ...
    ) -> Result<CounterSampleBuffer, metal_foundation::Error> {
        unsafe {
    ...
    pub fn counter_sets_raw(&self) -> *mut c_void {
        unsafe { metal_sys::msg_send_0(self.as_ptr(), sel!(counterSets)) }
    }
    ...
    pub fn counter_set_count(&self) -> UInteger {
        unsafe {
            ...
            metal_sys::msg_send_0(array as *const c_void, sel!(count))
        }
    }
    ...
    pub fn counter_set_at_index(&self, index: UInteger) -> Option<CounterSet> {
        unsafe {
            ...
            metal_sys::msg_send_0::<*mut c_void>(ptr as *const c_void, sel!(retain));
    ...
    ) -> Result<LogState, metal_foundation::Error> {
        unsafe {
            let mut error: *mut c_void = std::ptr::null_mut();
            let ptr: *mut c_void = msg_send_2(
                self.as_ptr(),
    ...
    ) -> Result<TextureViewPool, metal_foundation::Error> {
        unsafe {
            let mut error: *mut c_void = std::ptr::null_mut();
            let ptr: *mut c_void = msg_send_2(
                self.as_ptr(),
    ...
    ) -> Result<Tensor, metal_foundation::Error> {
        unsafe {
            let mut error: *mut c_void = std::ptr::null_mut();
            let ptr: *mut c_void = msg_send_2(
                self.as_ptr(),
